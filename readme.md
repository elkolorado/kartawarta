...
# Docker load testing and VPS sizing

Use the monitoring stack and k6 tests in [`loadtests/README.md`](loadtests/README.md) to measure real container CPU, RAM, network, disk I/O, and response times under load.

For VPS deployment with `kartawarta.pl`, host nginx, HTTPS, secrets, private ports, and safe monitoring, see [`docs/production.md`](docs/production.md).
...
Simulate a small 2 vCore / 4 GB VPS locally:

```bash
docker compose -f docker-compose.yml -f docker-compose.monitoring.yml -f docker-compose.vps-2vcpu-4gb.yml up -d --build
```
