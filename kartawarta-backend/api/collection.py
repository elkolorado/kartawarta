...
from .schemas import AddCardRequest, RemoveCardRequest, BulkCollectionRequest
from db import (
    get_user_collection,
...
def bulk_collection_action(req: BulkCollectionRequest, current_user: str = Depends(get_current_user)):
    ...
    if req.action == "add":
        return bulk_add_cards_to_user_collection(user_id, [item.model_dump() for item in req.items])

    return bulk_remove_cards_from_user_collection(user_id, [item.model_dump() for item in req.items])
